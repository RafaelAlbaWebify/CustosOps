...
class WindowsEventReportResponse(BaseModel):
    filename: str
    format: str
    content_type: str
    content: str
...
def build_windows_event_report(
    evidence: dict[str, Any],
    findings: list[dict[str, Any]],
    report_format: str,
) -> WindowsEventReportResponse:
    evidence = _as_dict(evidence)
    findings = [_as_dict(finding) for finding in findings]

    generated_at = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")
    source_file = str(evidence.get("source_file", "windows-events"))
    ...
    if report_format == "json":
        content = json.dumps(
            {
                "report_type": "custosops.windows_events.v0.1",
    ...
    return WindowsEventReportResponse(
        filename=f"custosops_windows_event_report_{stem}_{timestamp}.{extension}",
...
def _summary(evidence: dict[str, Any], findings: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "source_file": evidence.get("source_file", "windows-events"),
        "raw_event_count": evidence.get("raw_event_count", 0),
        "parsed_event_count": evidence.get("parsed_event_count", 0),
...
def _build_markdown_report(findings: list[dict[str, Any]], summary: dict[str, Any], generated_at: str) -> str:
    ...
    lines.append("# CustosOps Windows Event Evidence Report")
    lines.append("")
    lines.append(f"Generated: {generated_at}")
    lines.append(f"Source file: `{summary['source_file']}`")
    lines.append(f"Raw events: `{summary['raw_event_count']}`")
    lines.append(f"Parsed events: `{summary['parsed_event_count']}`")
    lines.append("")
...
<head>
  <meta charset="utf-8">
  <title>CustosOps Windows Event Evidence Report</title>
...
<main>
  <section class="hero">
    <p class="eyebrow">CustosOps Windows Event Evidence Report</p>
    <h1>Windows Event Evidence Report</h1>
...
def _html_finding(finding: dict[str, Any]) -> str:
  ...
  <div class="finding-top">
    <div>
      <p class="eyebrow">{html.escape(str(finding.get('category', 'Windows Event evidence')))}</p>
...
def _safe_stem(filename: str) -> str:
    stem = Path(filename).stem or "windows_events"
    cleaned = "".join(char if char.isalnum() or char in {"-", "_"} else "_" for char in stem)
    return cleaned[:80] or "windows_events"
